...
현재 문서 기준 버전은 `2026-07-16.78`입니다.
...
A* 금지 영역은 본체와 부속품을 모두 포함한 외접 반경에 `본체 외곽 추가 금지 여유`와 `장애물 추가 여유`를 더해 계산합니다. 따라서 경로선은 로봇 중심 기준으로 벽과 장애물에서 본체 반경만큼 추가로 떨어집니다. 그 바깥 Inflation 영역에는 높은 이동 비용을 적용하여 우회 공간이 있으면 벽에 붙는 최단 경로보다 여유 있는 경로를 우선합니다.
