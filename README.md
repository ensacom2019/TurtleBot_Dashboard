...
현재 문서 기준 버전은 `2026-07-22.97`입니다.
...
주행 맵 상단의 `노란 경유지 자동 보정`은 기본으로 켜져 있습니다. 노란 Inflation 셀에 찍은 새 경유지나 최종 목표는 노란 여유 영역을 막 벗어난 가장 가까운 흰색 셀로 옮기고, 화면에 보정 전·후 좌표를 표시합니다. 흰색 셀이 없으면 원래 노란 좌표를 유지하며 회색으로 표시합니다. 붉은 금지 셀은 기존처럼 보정하지 않고 경로 생성을 거부합니다.
